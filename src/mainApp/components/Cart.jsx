...
function Cart()
{
    ...
    <div className="name-date">
        <h1> {product.name}</h1>      
        <h2 style = {{fontSize: 12}}>Rating: {product.rating}</h2>
        <div className="qty-count">
        <div className="quantity">
            <FontAwesomeIcon icon={faMinus} className = "faFade" onClick = {() => {  if(product.productCount > 0) {dispatch(decrement(index));}}} />
            <span className='counter'> {product.productCount}</span>
            <FontAwesomeIcon icon={faPlus}  onClick = {() => { if(product.productCount < product.countInStock) {dispatch(increment(index))}}}/>
        </div>
        </div>
    </div>
    </div>
    <h2 className = "price"><span>Price : &#8377;{product.price * product.productCount}</span></h2>
        ...
        return (
            ...
            <div className="orders">
                ...
                <div className="place-order-button">
                    <button type="button" className="remove" onClick = {()=>{dispatch(Empty()); console.log(cart);}}>Remove</button>
                    <button type="button" className="place-order" onClick = {()=> {handleBuy(); dispatch(Empty())}}>Place Order</button>
                </div>
            </div>
